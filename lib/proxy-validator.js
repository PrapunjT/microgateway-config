...
module.exports.validate = function validate(configObject, options) {
  ...
  config.proxies.forEach(function (proxy, index) {
    ...
    assert(typeof proxy.max_connections === CONSTANTS.NUMBER, 'config.proxy[' + index + '].max_connections is not a number');
  });
  if(!config.edgemicro.hasOwnProperty('enableAnalytics') || config.edgemicro.enableAnalytics === true || options.metrics === true){
    if (config.analytics) {
      ...
      assert(typeof config.analytics.uri === CONSTANTS.STRING, 'config.analytics.uri is not a string');
      ...
      assert(typeof config.analytics.proxy_revision === CONSTANTS.NUMBER, 'config.analytics.proxy_revision is not a number');
    }
  }
  if (config.oauth) {
    assert(typeof config.oauth.public_key === CONSTANTS.STRING, 'config.oauth.public_key is not defined');
  }
  if (config.apikeys) {
    assert(typeof config.apikeys.public_key === CONSTANTS.STRING, 'config.apikeys.public_key is not defined');
  }
  if (config.oauthv2) {
    assert(typeof config.oauthv2.public_key === CONSTANTS.STRING, 'config.oauthv2.public_key is not defined');
  }  
  return configObject;
};
