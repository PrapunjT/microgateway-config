...
IO.prototype.initConfig = function (options, cb) {
  ...
  fs.ensureDir(configDir, function (err) {
    //
    if ( err ) writeConsoleLog(CONSTANTS.LOGGING.ERROR,{component: CONSOLE_LOG_TAG_COMP},err);
    //
    fs.stat(configPath, function (err /*, stats */) {
      if (err) {
        writeConsoleLog(CONSTANTS.LOGGING.LOG,{component: CONSOLE_LOG_TAG_COMP},"file doesn't exist, setting up");
        ...
        fs.copy(source, configPath, {clobber: overwrite}, function (err) {
          if(err) {
            writeConsoleLog(CONSTANTS.LOGGING.ERROR,{component: CONSOLE_LOG_TAG_COMP},err);
            return cb(err);
          }
          return cb(err,configPath);
        });
...
IO.prototype.loadSync = function (options) {
  ...
  if (!fs.existsSync(source)) {
    writeConsoleLog(CONSTANTS.LOGGING.ERROR,{component: CONSOLE_LOG_TAG_COMP},'config does not exist', source);
    throw new Error('config does not exist');
  }
  const stat = fs.statSync(source);
  if (!stat.isFile()) {
    writeConsoleLog(CONSTANTS.LOGGING.ERROR,{component: CONSOLE_LOG_TAG_COMP},'config is not a file', source);
    throw new Error('config is not a file');
  }
  if (stat.size === 0) {
    writeConsoleLog(CONSTANTS.LOGGING.ERROR,{component: CONSOLE_LOG_TAG_COMP},'config is empty', source);
  ...
  try {
    ...
    writeConsoleLog(CONSTANTS.LOGGING.ERROR,{component: CONSOLE_LOG_TAG_COMP},'error reading config from', source, err);
    throw new Error(err);
  }
...
IO.prototype.save = function (config, options, cb) {
  ...
  if (options.sync) {
    ...
    fs.writeFile(target, dump, function (err) {
      if ( err ) writeConsoleLog(CONSTANTS.LOGGING.ERROR,{component: CONSOLE_LOG_TAG_COMP},'error saving config to', target, err);
