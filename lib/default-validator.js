...
module.exports.validate = function validate(configObject, options={}) {
  ...
  if (config.edgemicro.on_target_response_abort) {
    assert(typeof config.edgemicro.on_target_response_abort === 'string', 'config.edgemicro.on_target_response_abort should be a string');
    assert(config.edgemicro.on_target_response_abort === 'appendErrorToClientResponseBody' || config.edgemicro.on_target_response_abort === 'abortClientRequest', 'config.edgemicro.on_target_response_abort should be either appendErrorToClientResponseBody | abortClientRequest');
  }

  if(config.extauth){
    if(config.extauth.rotate_public_key){
      assert(typeof config.extauth.rotate_public_key === 'boolean', 'config.extauth.rotate_public_key should be boolean');
    }
  }
