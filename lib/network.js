...
Loader.prototype.get = function(options, callback) {
    ...
    if (process.env.EDGEMICRO_LOCAL === "1") {
        ...
        if ( (typeof configurl !== CONSTANTS.UNDEFINED) && configurl) {
            request.get(configurl, this, function(error, response, body) {
                if (!error && response.statusCode === 200) {
                    writeConsoleLog(CONSTANTS.LOGGING.LOG,{component: CONSOLE_LOG_TAG_COMP},'downloading configuration from: ' + configurl);
                    debug(body);
                    ...
                    writeConsoleLog(CONSTANTS.LOGGING.WARN,{component: CONSOLE_LOG_TAG_COMP},'using old cached configuration');
                }
...
function writeConfig(source, body) {
    try {
        ...
        writeConsoleLog(CONSTANTS.LOGGING.ERROR,{component: CONSOLE_LOG_TAG_COMP},'Error: ' + err);
    }
}
...
function loadConfiguration(thisconfig, keys, callback, envReplacedConfig) {
    ...
    _load(envReplacedConfig, keys, function(err, proxies, products) {
        ...
        if (proxies && products) {
            ...
            if (mergedConfig) {
                writeConsoleLog(CONSTANTS.LOGGING.INFO,{component: CONSOLE_LOG_TAG_COMP},'loaded cached config from', target);
                callback(null, mergedConfig);
            } else {
                writeConsoleLog(CONSTANTS.LOGGING.ERROR,{component: CONSOLE_LOG_TAG_COMP},'fatal:',
...
function validateJSON(data){
    if(typeof data === CONSTANTS.STRING){
        ...
        return true;
    }else if(typeof data === CONSTANTS.OBJECT){
        return true;
    }else{
        return false;
    }
...
const processLoad = (config, keys, callback, useSynchronizer, redisClient, sourceConfig) => {
    ...
    if (typeof config.edge_config.proxyPattern !== CONSTANTS.UNDEFINED) {
    ...
    async.parallel([
        function(cb) {
            ...
            if (process.env.EDGEMICRO_LOCAL_PROXY === "1") {
                const proxyInfo = "{\"apiProxies\": [{\"apiProxyName\":\"" + config.proxies[0].name + "\"," +
                    ...
                    "}}]}";
                const response = {
                    statusCode: CONSTANTS.HTTPSTATUS.HTTP_STATUS_OK
                };
                _loadStatus(CONSTANTS.REDIS.CONFIG, config.edge_config.bootstrap, null, response, proxyInfo, cb);
            } else  { //retrieve info from edge
                if ( useSynchronizer || !config.edge_config.redisBasedConfigCache ) {
                    request.get(opts, function(err, response, body) {
                        if(useSynchronizer && !err && response && response.statusCode === CONSTANTS.HTTPSTATUS.HTTP_STATUS_OK && validateJSON(body)){
                            saveConfigToRedis(redisClient, globalOptions, config.edge_config.bootstrap, body, CONSTANTS.REDIS.CONFIG, (err)=>{
                                if ( err ) {
                                    writeConsoleLog(CONSTANTS.LOGGING.ERROR,{component: CONSOLE_LOG_TAG_COMP}, 'error saving data to redis from %s', config.edge_config.bootstrap, err);
                                    return;
                                }
                                writeConsoleLog(CONSTANTS.LOGGING.INFO,{component: CONSOLE_LOG_TAG_COMP}, 'Saved data to redis from %s', config.edge_config.bootstrap);
                            });
                        }
                        if (!config.edge_config.redisBasedConfigCache) {
                            _loadStatus(CONSTANTS.REDIS.CONFIG, config.edge_config.bootstrap, err, response, body, cb);
                        }
                    });
                }  
                
                if ( config.edge_config.redisBasedConfigCache === true ) { //retrieve info from redis db
                    getConfigFromRedis(redisClient, globalOptions,config.edge_config.bootstrap, CONSTANTS.REDIS.CONFIG, function(err, body){
                        const response =  err ? null : { statusCode: CONSTANTS.HTTPSTATUS.HTTP_STATUS_OK, statusMessage: 'Downloaded from redis' };
                        _loadStatus(CONSTANTS.REDIS.CONFIG, config.edgemicro.redisHost, err, response, body, cb);
                    });
                } 
            ...
            if ( useSynchronizer || !config.edge_config.redisBasedConfigCache ) {
                ...
                request.get(opts, function(err, response, body) {
                    if(useSynchronizer && !err && response && response.statusCode === 200 && validateJSON(body)){
                        saveConfigToRedis(redisClient, globalOptions, config.edge_config.products, body, CONSTANTS.REDIS.PRODUCTS, (err)=>{
                            if ( err ) {
                                writeConsoleLog(CONSTANTS.LOGGING.ERROR,{component: CONSOLE_LOG_TAG_COMP}, 'error saving data to redis from %s', config.edge_config.products, err);
                                return;
                            }
                            writeConsoleLog(CONSTANTS.LOGGING.INFO,{component: CONSOLE_LOG_TAG_COMP}, 'Saved data to redis from %s', config.edge_config.products);
                        });
                    }
                    if (!config.edge_config.redisBasedConfigCache) {
                        _loadStatus(CONSTANTS.REDIS.PRODUCTS, config.edge_config.products, err, response, body, cb);
                    }
                });
            }
            
            if ( config.edge_config.redisBasedConfigCache === true ) { //retrieve info from redis db
                getConfigFromRedis(redisClient, globalOptions,config.edge_config.products, CONSTANTS.REDIS.PRODUCTS, function(err, body){
                    const response =  err ? null : { statusCode: CONSTANTS.HTTPSTATUS.HTTP_STATUS_OK, statusMessage: 'Downloaded from redis' };
                    _loadStatus(CONSTANTS.REDIS.PRODUCTS, config.edgemicro.redisHost, err, response, body, cb);
                });
            } 
        },
        function(cb) {
            if ( useSynchronizer || !config.edge_config.redisBasedConfigCache ) {
                ...
                request.get(opts, function(err, response, body) {
                    if(useSynchronizer && !err && response && response.statusCode === 200){
                        saveConfigToRedis(redisClient, globalOptions, config.edge_config.jwt_public_key, body, CONSTANTS.REDIS.OAUTH.JWTPK, (err)=>{
                            if ( err ) {
                                writeConsoleLog(CONSTANTS.LOGGING.ERROR,{component: CONSOLE_LOG_TAG_COMP}, 'error saving data to redis from %s', config.edge_config.jwt_public_key, err);
                                return;
                            }
                            writeConsoleLog(CONSTANTS.LOGGING.INFO,{component: CONSOLE_LOG_TAG_COMP}, 'Saved data to redis from %s', config.edge_config.jwt_public_key);
                        });
                    }
                    if ( !config.edge_config.redisBasedConfigCache) {
                        _loadStatus(CONSTANTS.REDIS.OAUTH.JWTPK, config.edge_config.jwt_public_key, err, response, body, cb);
                    }
                });
            }
            
            if ( config.edge_config.redisBasedConfigCache === true ) { //retrieve info from redis db
                getConfigFromRedis(redisClient, globalOptions, config.edge_config.jwt_public_key, CONSTANTS.REDIS.OAUTH.JWTPK, function(err, body){
                    const response =  err ? null : { statusCode: CONSTANTS.HTTPSTATUS.HTTP_STATUS_OK, statusMessage: 'Downloaded from redis' };
                    _loadStatus(CONSTANTS.REDIS.OAUTH.JWTPK, config.edgemicro.redisHost, err, response, body, cb);
                });
            } 
        },
        function(cb) {
            ...
            if ( useSynchronizer || !config.edge_config.redisBasedConfigCache ) {
                ...
                opts = enableTLS(config, opts);
                request.get(opts, function(err, response, body) {
                    if(useSynchronizer && !err && response && response.statusCode === CONSTANTS.HTTPSTATUS.HTTP_STATUS_OK){
                        saveConfigToRedis(redisClient, globalOptions, config.edge_config.jwk_public_keys, body, CONSTANTS.REDIS.OAUTH.PK, (err)=>{
                            if ( err ) {
                                writeConsoleLog(CONSTANTS.LOGGING.ERROR,{component: CONSOLE_LOG_TAG_COMP}, 'error saving data to redis from %s', config.edge_config.jwk_public_keys, err);
                                return;
                            }
                            writeConsoleLog(CONSTANTS.LOGGING.INFO,{component: CONSOLE_LOG_TAG_COMP}, 'Saved data to redis from %s', config.edge_config.jwk_public_keys);
                        });
                    }
                    if ( !config.edge_config.redisBasedConfigCache) {
                        _loadStatus(CONSTANTS.REDIS.OAUTH.PK, config.edge_config.jwk_public_keys, err, response, body, cb);
                    }
                });
            }

            if ( config.edge_config.redisBasedConfigCache === true ) { //retrieve info from redis db
                getConfigFromRedis(redisClient, globalOptions, config.edge_config.jwk_public_keys, CONSTANTS.REDIS.OAUTH.PK, function(err, body){
                    const response =  err ? null : { statusCode: CONSTANTS.HTTPSTATUS.HTTP_STATUS_OK, statusMessage: 'Downloaded from redis' };
                    _loadStatus(CONSTANTS.REDIS.OAUTH.PK, config.edgemicro.redisHost, err, response, body, cb);
                });
            } 
        },
        // fetch extauth public key
        function(cb) {
            if(!config.edgemicro.plugins.sequence.includes("extauth") || !config.extauth || !config.extauth.publickey_url){
                return cb(null,null);
            }
            if ( useSynchronizer || !config.edge_config.redisBasedConfigCache ) {
                var opts = _.clone(options);
                ...
                request.get(opts, function(err, response, body) {
                    if(useSynchronizer && !err && response && response.statusCode === CONSTANTS.HTTPSTATUS.HTTP_STATUS_OK){
                        saveConfigToRedis(redisClient, globalOptions, config.extauth.public_keys, body, CONSTANTS.REDIS.EXTAUTH.PK, (err)=>{
                            if ( err ) {
                                writeConsoleLog(CONSTANTS.LOGGING.ERROR,{component: CONSOLE_LOG_TAG_COMP}, 'error saving data to redis from %s', config.extauth.public_keys, err);
                                return;
                            }
                            writeConsoleLog(CONSTANTS.LOGGING.INFO,{component: CONSOLE_LOG_TAG_COMP}, 'Saved data to redis from %s', config.extauth.public_keys);
                        });
                    }
                    if ( !config.edge_config.redisBasedConfigCache) {
                        _loadStatus(CONSTANTS.REDIS.EXTAUTH.PK, config.extauth.publickey_url, err, response, body, cb);
                    }
                });
            }

            if ( config.edge_config.redisBasedConfigCache === true ) { //retrieve info from redis db
                getConfigFromRedis(redisClient, globalOptions, config.extauth.public_keys, CONSTANTS.REDIS.EXTAUTH.PK, function(err, body){
                    const response =  err ? null : { statusCode: CONSTANTS.HTTPSTATUS.HTTP_STATUS_OK, statusMessage: 'Downloaded from redis' };
                    _loadStatus(CONSTANTS.REDIS.EXTAUTH.PK, config.edgemicro.redisHost, err, response, body, cb);
                });
            }
        }
        ...
        if (err) {
            writeConsoleLog(CONSTANTS.LOGGING.WARN,{component: CONSOLE_LOG_TAG_COMP},
        ...
        try {
            ...
            writeConsoleLog(CONSTANTS.LOGGING.ERROR,{component: CONSOLE_LOG_TAG_COMP},'CRITICAL ERROR:', 'error parsing downloaded proxy list',
                err);
            return callback(err);
        }
        var proxies = proxyInfo && proxyInfo.apiProxies ? proxyInfo.apiProxies : [];
        if (!proxies) {
            writeConsoleLog(CONSTANTS.LOGGING.WARN,{component: CONSOLE_LOG_TAG_COMP},'no edge micro proxies found in response');
        }
        if (proxies.length === 0) {
            writeConsoleLog(CONSTANTS.LOGGING.WARN,{component: CONSOLE_LOG_TAG_COMP}, 'no edge micro proxies found in org');
        }
        ...
        try {
            ...
            if(err instanceof SyntaxError) writeConsoleLog(CONSTANTS.LOGGING.ERROR,{component: CONSOLE_LOG_TAG_COMP},'CRITICAL ERROR:', 'error parsing downloaded product list',
                err);
            return callback(new Error('CRITICAL ERROR: error parsing downloaded product list'));
        }
        if (!productInfo) {
            writeConsoleLog(CONSTANTS.LOGGING.WARN,{component: CONSOLE_LOG_TAG_COMP},'no edge micro products found in response');
        }
        var products = productInfo && productInfo.apiProduct ? productInfo.apiProduct : [];
        if (!products) {
            writeConsoleLog(CONSTANTS.LOGGING.WARN,{component: CONSOLE_LOG_TAG_COMP}, 'no products found in response');
            products = [];
        }
        if (products.length === 0) {
            writeConsoleLog(CONSTANTS.LOGGING.WARN,{component: CONSOLE_LOG_TAG_COMP},'no products found in org');
        }
        /**
        ...
        if (results.length > 1 && results[2]) {
            ...
            writeConsoleLog(CONSTANTS.LOGGING.WARN,{component: CONSOLE_LOG_TAG_COMP},'failed to download jwt_public_key');
        }
        //add jwk support
        try {
            ...
            writeConsoleLog(CONSTANTS.LOGGING.WARN,{component: CONSOLE_LOG_TAG_COMP},'jwk keys are not enabled');
        }
        //add jwk support
...
const _loadStatus = function(message, url, err, response, body, cb) {
    ...
    if (url) {
        ...
        if ( failed ) {
            writeConsoleLog(CONSTANTS.LOGGING.WARN,{component: CONSOLE_LOG_TAG_COMP},message, 'download from', url,'returned',
                            (response ? (response.statusCode + ' ' + response.statusMessage) : '', err ? err : ''));
        } else {
            writeConsoleLog(CONSTANTS.LOGGING.INFO,{component: CONSOLE_LOG_TAG_COMP},message, 'download from', url, 'returned',
                            (response ? (response.statusCode + ' ' + response.statusMessage) : ''), err ? err : '');
        }
    }
    if (err) {
...
const _validateUrls = function(config) {
    ...
    if (bootstrapUrl.hostname === 'apigee.net' ||
        ...
        writeConsoleLog(CONSTANTS.LOGGING.ERROR,{component: CONSOLE_LOG_TAG_COMP},'it looks like edge micro has not been configured, please see the admin guide');
...
function saveConfigToRedis(redisClient, options, proxy, data, suffixKey, cb) {
    if(!data){
        writeConsoleLog(CONSTANTS.LOGGING.WARN,{component: CONSOLE_LOG_TAG_COMP},'Response Data is empty. Unable to write in Redis DB.');
    ...
    try {
        ...
        if ( proxyName && typeof proxyName === CONSTANTS.STRING ){
            key += REDIS_KEY_ORG_ENV_DELIMETER + proxyName;
        }
        if ( suffixKey && typeof suffixKey === CONSTANTS.STRING ){
