...
const redisClientConnection = function(config, cb) {
    let host = config.redisHost || CONSTANTS.REDIS.HOST_DEFAULT;
    let port = config.redisPort || CONSTANTS.REDIS.PORT_DEFAULT;
    let db = config.redisDb || CONSTANTS.REDIS.DB_DEFUALT;
